...
si5351::begin() //didn't like being inlined
{
  ...
  //we cannot clear register 1 yet because they will immedietly set if we don't handle register 2 first.
  writeRegister(2, 0x10);//Register 2 is the mask bits. Disable only the loss of clock in bit, as that is for the SI5351C only.
  ...
  writeRegister(9, 0xFF);//OEB doesn't control shit because it DNE on the 10-MSOP package, so disable its influence. Disabled for the 20-QFN package too for simplicity.
  ...
  SI5351_updateClkCont(0, 1); // configure output 0 and turn off output 0
  SI5351_updateClkCont(1, 1); // configure output 1 and turn off output 1
  SI5351_updateClkCont(2, 1); // configure output 2 and turn off output 2
  SI5351_updateClkCont(3, 1); // configure output 3 and turn off output 3
  SI5351_updateClkCont(4, 1); // configure output 4 and turn off output 4
  SI5351_updateClkCont(5, 1); // configure output 5 and turn off output 5
...
//TODO: Verify spread spectrum performance.
...
si5351::spreadSpectrum(float percent, SpreadType spreadType)
{
  ...
  if((0 == percent)||(spreadType_disabled == spreadType))//check if the requested spread is 0 or spreadType is spreadType_disabled. If so, turn off spread spectrum.
  {
    //turn off the spread spectrum
      ...
      writeRegister(22, 0b11000000); //set PLL A to int mode, decreasing the phase noise.
  ...
  else
  {
    ...
    uint8_t SSUP_P2[2];
    uint8_t SSUP_P3[2];
    float sscAmp = percent / 100; //this is the amplitude of the spread spectrum modulation in a decimal format

    //Calculate and set shared parameters
    SSUDP[1] = 0x00; // based on the XO freq. Do not change.
    SSUDP[0] = 0xC6; // based on the XO freq. Do not change.
...
//TODO: Figure out better way to handle this calculation than a recursive while loop
/*
 ...
 */
void //works // taking about 1.05 ms which is understandable
